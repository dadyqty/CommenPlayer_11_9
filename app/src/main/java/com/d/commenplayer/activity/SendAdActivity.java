...
public class SendAdActivity extends Activity {
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        if (event.getKeyCode() == 216 && event.getAction() == KeyEvent.ACTION_UP)       //取消键 KeyCode() == 216, 表示返回
        {
            ...
            finish();
            ToastUtile.showText(SendAdActivity.this, "返回主界面");
        ...
        if (event.getKeyCode() == 215&& event.getAction() == KeyEvent.ACTION_UP)       //确定键  KeyCode() == 215, 表示发送短信
        {
            ...
            if(zhongduan.length()<9){
                ...
                String CheckData="01B3"+Length+"2F"+"0"+zhongduan+msglen+"02"+gbk;
    ...
    private class MyHandle extends Handler {
        private final WeakReference<SendAdActivity> mActivity;
        public MyHandle(SendAdActivity activity){
            mActivity = new WeakReference<>(activity);
        }
        @Override
        public void handleMessage(Message msg) {
            SendAdActivity activity = mActivity.get();
            if (activity != null){
                switch (msg.what){
                    case 1:
                        new Thread(){
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what){
                case WHAT_SEND_ERROR:
                    if(count<9){
                        count++;
                        ToastUtile.showText(SendAdActivity.this, "消息正在重新发送中。。。");
                        sendData(SendData);
        ...
        if(data_use){
            ...
            switch (cmd) {
                ...
                case "29":
                    ...
                    MsgInfo msginfopre29=new MsgInfo(-1,address.getText().toString(),shortmessage.getText().toString()," "," "," "," ",3,0);
